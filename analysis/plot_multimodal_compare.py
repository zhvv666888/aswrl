...
def load_sync_error_from_db(db_path):
    ...
    # 🔍 自动查找 sync status topic（鲁棒）
    cur.execute("""
        SELECT id, name FROM topics
        WHERE name LIKE '%sync%status%'
    """)
    row = cur.fetchone()
    if row is None:
        raise RuntimeError(f"No sync status topic found in {db_path}")

    topic_id, real_name = row
    print(f"[INFO] Using topic '{real_name}' from {db_path}")
    ...
    for ts, blob in cur.fetchall():
        ...
        err_ms = values[2] * 1e-3  # index=2, us -> ms
        t_list.append(t)
...
for label, db in BAGS.items():
    ...
    plt.plot(t, err_dt, label=label, linewidth=1.3, alpha=0.8)

plt.xlabel("Time (s)")
plt.ylabel("Detrended Sync Error (ms)")
plt.title("Multimodal Network Scenario")
plt.legend()
...
plt.boxplot(
    [errors_for_box[k] for k in ["FW", "Rule-ASW"]],
    labels=["FW", "Rule-ASW"],
    showfliers=False,
)

plt.ylabel("Detrended Sync Error (ms)")
plt.title("Quantitative Comparison under Multimodal Scenario")
...
# ====== 3. 打印指标 ======
print("=== Multimodal Scenario Metrics ===")
